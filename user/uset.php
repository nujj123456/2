...
<?php
	...
	if($sitename==null){
		...
		if(Template::exists($template)==false)exit("<script language='javascript'>alert('该模板首页文件不存在！');history.go(-1);</script>");
		$sds=$DB->query("update shua_site set sitename='$sitename',title='$title',keywords='$keywords',description='$description',kaurl='$kaurl',anounce='$anounce',modal='$modal',bottom='$bottom',ktfz_price='$ktfz_price',ktfz_price2='$ktfz_price2',ktfz_domain='$ktfz_domain',template='$template' where zid='{$userrow['zid']}'");
...
copy($_FILES['file']['tmp_name'], ROOT.'assets/img/logo_'.$userrow['zid'].'.'.$ext);
echo "成功上传文件!<br>（可能需要清空浏览器缓存才能看到效果）";
}
if(file_exists(ROOT.'assets/img/logo_'.$userrow['zid'].'.'.$ext)){
	$logo = '../assets/img/logo_'.$userrow['zid'].'.'.$ext;
}else{
...
copy($_FILES['shoukuan']['tmp_name'], ROOT.'assets/img/skimg/sk_'.$userrow['zid'].'.'.$ext);
echo "成功上传文件!<br>（可能需要清空浏览器缓存才能看到效果）";
}
if(file_exists(ROOT.'assets/img/skimg/sk_'.$userrow['zid'].'.'.$ext)){
	$logo = '../assets/img/skimg/sk_'.$userrow['zid'].'.'.$ext;
}else{
